...
function getParent<T>(node: Node<T> | null): Node<T> | null {
    return !node ? node : node.parent;
}

function getGrandParent<T>(node: Node<T> | null): Node<T> | null {
    return getParent(getParent(node));
}

function getSibling<T>(node: Node<T> | null): Node<T> | null {
...
function getUncle<T>(node: Node<T> | null): Node<T> | null {
    return getSibling(getParent(node));
}

function maybe<T>(value: T | null, callback: (value: T) => void): void {
    if (value) {
        callback(value);
    }
} 

function isOnLeft<T>(node: Node<T> | null): boolean {
    return node?.parent?.left === node;
}

function hasRedChild<T>(node: Node<T> | null): boolean {
    return node?.left?.color === Color.RED || node?.right?.color === Color.RED;
}

function swapValues<T>(firstNode: Node<T>, secondNode: Node<T>): void {
    let temp: T;
...
export class Tree<T> {
    ...
    public delete(key: number): Node<T> | null {
    ...
    private getLastNode(node: Node<T> | null = this.root): Node<T> | null{
